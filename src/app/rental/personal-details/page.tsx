...
import { isValidIsraeliMobile } from "../../../lib/rentals/phone";
...
export default function PersonalDetailsPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!isValidIsraeliMobile(phone1) || !isValidIsraeliMobile(phone2)) {
      setPhoneError(
        "נא להזין שני מספרי נייד ישראליים תקינים (למשל 055-677-1200)",
      );
      return;
    }
    setPhoneError(null);
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  setPhoneError(null);
                  updatePersonal({ phone1: e.target.value });
                }}
                className="mt-1 w-full rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-right text-foreground focus:border-brand focus:ring-2 focus:ring-brand/20"
                placeholder="055-677-1200"
              />
            </div>
            <div>
              <label
              ...
              <input
                ...
                onChange={(e) => {
                  setPhoneError(null);
                  updatePersonal({ phone2: e.target.value });
                }}
                className="mt-1 w-full rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-right text-foreground focus:border-brand focus:ring-2 focus:ring-brand/20"
