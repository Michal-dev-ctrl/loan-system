...
const HEARTS_SUB_ITEMS: { id: (typeof HEARTS_SUB_IDS)[number]; name: string }[] = [
  { id: "happy-hearts-1", name: "לבבות גדולים" },
  { id: "happy-hearts-2", name: "לבבות קטנים" },
];

const IMAGE_EXTENSIONS = [".png", ".PNG", ".jpg", ".jpeg", ".JPG", ".JPEG", ".webp", ".WEBP"];

function HeartImage({ itemId, name }: { itemId: string; name: string }) {
...
export default function HeartsPage() {
  ...
  const heartsTotal = HEARTS_SUB_IDS.reduce((s, id) => s + (draft.items[id] ?? 0), 0);

  const setHeartsQuantity = (id: (typeof HEARTS_SUB_IDS)[number], value: number) => {
            ...
            {HEARTS_SUB_ITEMS.map(({ id, name }) => {
              ...
              return (
                <div key={id} className={cardBase + (qty > 0 ? cardSelected : "")}>
                  ...
                  <HeartImage itemId={id} name={name} />
                  <div className="mt-1.5 text-right">
                    ...
                    <div className="flex items-center justify-end gap-1.5 mt-1">
                      <button
                        type="button"
                        className={qtyBtn}
                        onClick={() => setHeartsQuantity(id, qty - 1)}
                      >
                        −
                      </button>
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (!Number.isNaN(v)) setHeartsQuantity(id, v);
                        }}
