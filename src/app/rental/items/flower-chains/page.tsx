...
const FLOWER_CHAINS_SUB_ITEMS: { id: (typeof FLOWER_CHAINS_SUB_IDS)[number]; name: string }[] = [
  { id: "happy-flower-chains-1", name: "שרשראות פרחים – פשוט" },
  { id: "happy-flower-chains-2", name: "שרשראות פרחים – יקר" },
];

const IMAGE_EXTENSIONS = [".png", ".PNG", ".jpg", ".jpeg", ".JPG", ".JPEG", ".webp", ".WEBP"];

function FlowerChainImage({ itemId, name }: { itemId: string; name: string }) {
...
export default function FlowerChainsPage() {
  const { draft, setItemQuantity } = useRentalDraft();

  const flowerChainsTotal = FLOWER_CHAINS_SUB_IDS.reduce((s, id) => s + (draft.items[id] ?? 0), 0);

  const setFlowerChainsQuantity = (id: (typeof FLOWER_CHAINS_SUB_IDS)[number], value: number) => {
  ...
  return (
    <div className="app-page text-foreground">
      <AppHeader
        ...
        rightSlot={`סה״כ ${flowerChainsTotal}`}
      />
      ...
      <main className="mx-auto flex max-w-4xl flex-col items-stretch px-4 py-8">
        <div className="rounded-2xl border border-brand-soft/60 bg-white p-6 shadow-[0_4px_20px_rgba(200,90,108,0.08)]">
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {FLOWER_CHAINS_SUB_ITEMS.map(({ id, name }) => {
              ...
              return (
                <div key={id} className={cardBase + (qty > 0 ? cardSelected : "")}>
                  ...
                  <FlowerChainImage itemId={id} name={name} />
                  <div className="mt-1.5 text-right">
                    ...
                    <div className="flex items-center justify-end gap-1.5 mt-1">
                      <button
                        type="button"
                        className={qtyBtn}
                        onClick={() => setFlowerChainsQuantity(id, qty - 1)}
                      >
                        −
                      </button>
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (!Number.isNaN(v)) setFlowerChainsQuantity(id, v);
                        }}
