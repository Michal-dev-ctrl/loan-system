...
const FANS_SUB_ITEMS: { id: (typeof FANS_SUB_IDS)[number]; name: string }[] = [
  { id: "happy-fans-1", name: "מניפות גדולות" },
  { id: "happy-fans-2", name: "מניפות קטנות" },
  { id: "happy-fans-3", name: "מניפות פרווה" },
];

const IMAGE_EXTENSIONS = [".png", ".PNG", ".jpg", ".jpeg", ".JPG", ".JPEG", ".webp", ".WEBP"];

function FanImage({ itemId, name }: { itemId: string; name: string }) {
...
export default function FansPage() {
  ...
  const fansTotal = FANS_SUB_IDS.reduce((s, id) => s + (draft.items[id] ?? 0), 0);

  const setFansQuantity = (id: (typeof FANS_SUB_IDS)[number], value: number) => {
                  ...
                  <div className="mt-1.5 text-right">
                    ...
                    <div className="flex items-center justify-end gap-1.5 mt-1">
                      <button
                        type="button"
                        className={qtyBtn}
                        onClick={() => setFansQuantity(id, qty - 1)}
                      >
                        −
                      </button>
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (!Number.isNaN(v)) setFansQuantity(id, v);
                        }}
