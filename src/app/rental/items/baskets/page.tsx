...
const BASKET_SUB_ITEMS: { id: (typeof BASKET_SUB_IDS)[number]; name: string }[] = [
...
export default function BasketsPage() {
  ...
  const basketTotal = BASKET_SUB_IDS.reduce((s, id) => s + (draft.items[id] ?? 0), 0);
  const basketAtMax = basketTotal >= BASKET_MAX_TOTAL;

  const setBasketQuantity = (id: (typeof BASKET_SUB_IDS)[number], value: number) => {
    const others = BASKET_SUB_IDS.filter((i) => i !== id).reduce(
      (s, i) => s + (draft.items[i] ?? 0),
      0
    );
    setItemQuantity(id, Math.min(Math.max(0, value), BASKET_MAX_TOTAL - others));
  };
  ...
  return (
    <div className="app-page text-foreground">
      <AppHeader
        backHref="/rental/items?category=basic"
        backLabel="לבחירת ציוד"
        rightSlot={`${basketTotal} / ${BASKET_MAX_TOTAL}`}
      />
      ...
      <main className="mx-auto flex max-w-4xl flex-col items-stretch px-4 py-8">
        <div className="rounded-2xl border border-brand-soft/60 bg-white p-6 shadow-[0_4px_20px_rgba(200,90,108,0.08)]">
          ...
          <p className="mt-2 text-center text-sm text-brand-dark">
            בחרו כמות לכל סוג. עד {BASKET_MAX_TOTAL} סלסלאות בסה״כ.
          </p>

          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {BASKET_SUB_ITEMS.map(({ id, name }) => {
                  ...
                  <div className="mt-1.5 text-right">
                    ...
                    <div className="flex items-center justify-end gap-1.5 mt-1">
                      <button
                        type="button"
                        className={qtyBtn}
                        onClick={() => setBasketQuantity(id, qty - 1)}
                      >
                        −
                      </button>
                      <input
                        type="number"
                        min={0}
                        max={BASKET_MAX_TOTAL}
                        value={qty}
                        onChange={(e) => {
                          const v = parseInt(e.target.value, 10);
                          if (!Number.isNaN(v)) setBasketQuantity(id, v);
                        }}
          ...
          <div className="mt-8 flex justify-center">
            <Link
              href="/rental/items?category=basic"
