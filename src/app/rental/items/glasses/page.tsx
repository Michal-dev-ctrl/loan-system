...
const GLASSES_SUB_IDS = [
  "happy-glasses-light",
  "happy-glasses-no-light",
] as const;

const GLASSES_SUB_ITEMS: {
  id: (typeof GLASSES_SUB_IDS)[number];
  name: string;
}[] = [
...
export default function GlassesPage() {
  ...
  const glassesTotal = GLASSES_SUB_IDS.reduce(
    (s, id) => s + (draft.items[id] ?? 0),
    0,
  );

  const setGlassesQuantity = (
    id: (typeof GLASSES_SUB_IDS)[number],
    value: number,
  ) => {
      ...
      <main className="mx-auto flex max-w-4xl flex-col items-stretch px-4 py-8">
        <div className="rounded-2xl border border-brand-soft/60 bg-white p-6 shadow-[0_4px_20px_rgba(200,90,108,0.08)]">
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {GLASSES_SUB_ITEMS.map(({ id, name }) => {
              const qty = draft.items[id] ?? 0;
              return (
                <div
                  key={id}
                  className={cardBase + (qty > 0 ? cardSelected : "")}
                >
                  {qty > 0 && (
                    <span
                      className="absolute top-2 right-2 z-10 flex h-7 w-7 items-center justify-center rounded-full bg-green-500 text-white text-sm font-bold shadow-md"
                      aria-hidden
                    >
                      ✓
                    </span>
                  ...
                  <div className="mt-1.5 text-right">
                    ...
                    <div className="flex items-center justify-end gap-1.5 mt-1">
                      <button
                        type="button"
                        className={qtyBtn}
                        onClick={() => setGlassesQuantity(id, qty - 1)}
                      >
                        −
                      </button>
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (!Number.isNaN(v)) setGlassesQuantity(id, v);
                        }}
