...
const STICKS_SUB_ITEMS: { id: (typeof STICKS_SUB_IDS)[number]; name: string }[] = [
  { id: "happy-sticks-threads-1", name: "מקלות צבעי אש" },
  { id: "happy-sticks-threads-2", name: "סרט לב אדום" },
  { id: "happy-sticks-threads-3", name: "סרט לב ורוד" },
  { id: "happy-sticks-threads-4", name: "מקלות צבעים סגול לבן" },
];

const IMAGE_EXTENSIONS = [".png", ".PNG", ".jpg", ".jpeg", ".JPG", ".JPEG", ".webp", ".WEBP"];

function SticksImage({ itemId, name }: { itemId: string; name: string }) {
...
export default function SticksThreadsPage() {
  const { draft, setItemQuantity } = useRentalDraft();

  const sticksTotal = STICKS_SUB_IDS.reduce((s, id) => s + (draft.items[id] ?? 0), 0);

  const setSticksQuantity = (id: (typeof STICKS_SUB_IDS)[number], value: number) => {
            ...
            {STICKS_SUB_ITEMS.map(({ id, name }) => {
              ...
              return (
                <div key={id} className={cardBase + (qty > 0 ? cardSelected : "")}>
                  ...
                  <SticksImage itemId={id} name={name} />
                  <div className="mt-1.5 text-right">
                    ...
                    <div className="flex items-center justify-end gap-1.5 mt-1">
                      <button
                        type="button"
                        className={qtyBtn}
                        onClick={() => setSticksQuantity(id, qty - 1)}
                      >
                        −
                      </button>
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (!Number.isNaN(v)) setSticksQuantity(id, v);
                        }}
