...
const INSTRUMENTS_SUB_ITEMS: { id: (typeof INSTRUMENTS_SUB_IDS)[number]; name: string }[] = [
  { id: "happy-instruments-1", name: "דרבוקות" },
  { id: "happy-instruments-2", name: "גיטרה" },
  { id: "happy-instruments-3", name: "תוף מרים" },
  { id: "happy-instruments-4", name: "קשקשנים" },
];

const IMAGE_EXTENSIONS = [".png", ".PNG", ".jpg", ".jpeg", ".JPG", ".JPEG", ".webp", ".WEBP"];

function InstrumentImage({ itemId, name }: { itemId: string; name: string }) {
...
export default function InstrumentsPage() {
  ...
  const instrumentsTotal = INSTRUMENTS_SUB_IDS.reduce((s, id) => s + (draft.items[id] ?? 0), 0);

  const setInstrumentsQuantity = (id: (typeof INSTRUMENTS_SUB_IDS)[number], value: number) => {
  ...
  return (
    <div className="app-page text-foreground">
      <AppHeader
        ...
        rightSlot={`סה״כ ${instrumentsTotal}`}
      />
      ...
      <main className="mx-auto flex max-w-4xl flex-col items-stretch px-4 py-8">
        <div className="rounded-2xl border border-brand-soft/60 bg-white p-6 shadow-[0_4px_20px_rgba(200,90,108,0.08)]">
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4">
            {INSTRUMENTS_SUB_ITEMS.map(({ id, name }) => {
              ...
              return (
                <div key={id} className={cardBase + (qty > 0 ? cardSelected : "")}>
                  ...
                  <InstrumentImage itemId={id} name={name} />
                  <div className="mt-1.5 text-right">
                    ...
                    <div className="flex items-center justify-end gap-1.5 mt-1">
                      <button
                        type="button"
                        className={qtyBtn}
                        onClick={() => setInstrumentsQuantity(id, qty - 1)}
                      >
                        −
                      </button>
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (!Number.isNaN(v)) setInstrumentsQuantity(id, v);
                        }}
